        ...
        if getattr(chat_request, "message_id", None):
            ...
            try:
                ...
                if ref_message.sources_used:
                    stored_used  = [json.dumps(s.__json__().__dict__) for s in ref_message.sources_used]
                else:
                    stored_used = []
                if ref_message.sources_notused:
                    stored_notused  = [json.dumps(s.__json__().__dict__) for s in ref_message.sources_notused]
                else:
                    stored_notused = []
