    ...
    try:
        ...
        if getattr(chat_request, "message_id", None) and getattr(chat_request, "filters", None):
            ref_msg = db.query(Message)\
                ...
                .first()
            ref_query = db.query(Message)\
                .filter(Message.id == chat_request.message_id-1)\
                .first()
            if not ref_msg:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
            ...
            try:
                ...
                summary_excluded = ref_message.summary_excluded or []
            except (json.JSONDecodeError, TypeError):
                context_chunks, context_chunks_notused, used_sources, unused_sources, summary_included, summary_excluded = [], [], [], [], [], []
        ...
        ai_message.summary_excluded = response_data.get('summary_excluded', [])
